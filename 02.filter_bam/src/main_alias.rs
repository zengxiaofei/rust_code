...
type MapqFilterFunc = fn(u8, u8, u8) -> bool;

fn mapq_filter_or(mapq1: u8, mapq2: u8, mapq: u8) -> bool {
    mapq1 >= mapq || mapq2 >= mapq
}

fn mapq_filter_and(mapq1: u8, mapq2: u8, mapq: u8) -> bool {
    mapq1 >= mapq && mapq2 >= mapq
}
...
fn parse_bam(args: &Cli) {
    ...
    let mapq_filter_func: MapqFilterFunc = if args.single_end_mapq_filtering {
        mapq_filter_or
    } else {
        mapq_filter_and
    };
    ...
    while let Some(line1) = next_line {
        ...
        if first_char == b'@' {
            ...
            if let Some(line2) = lines.next() {
                ...
                if mapq_filter_func(mapq1, mapq2, args.mapq) {
                    println!("{line1_str}\n{line2_str}");
                }
            } else {
                break;
